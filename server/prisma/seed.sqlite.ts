...
async function main() {
  console.log('🌱 Seeding SQLite offline development database...');
  ...
  console.log(`✅ Seeded ${Object.keys(createdCrops).length} crops`);
  ...
  console.log(`✅ Seeded ${mspPrices.length} MSP prices`);
  ...
  const centre3 = await prisma.procurementCentre.create({
    data: {
      name: 'Rajkot APMC Cotton & Groundnut Depot',
  ...
  await prisma.alert.create({
    data: {
      ...
      message: 'Procurement centres across Haryana and Punjab will open for Wheat MSP registration from April 1st. Ensure moisture content is below 12%.',
      priority: 'HIGH',
      cropId: createdCrops['Wheat'].id,
      location: 'Haryana',
      isPublished: true,
...
main()
  .catch((e) => {
    console.error('Error seeding SQLite:', e);
  })
  .finally(async () => {
